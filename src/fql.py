...
# -------------------------------
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim, activation=nn.GELU, activate_final=False):
        super().__init__()
        layers = []
...
class Critic(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden_dims, ensemble_size=2):
        ...
        self.nets = nn.ModuleList(
            [MLP(obs_dim + action_dim, hidden_dims, 1) for _ in range(ensemble_size)]
        )
...
def create_fql_agent(seed, ex_observations, ex_actions, config):
    """
